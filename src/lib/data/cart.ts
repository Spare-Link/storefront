...
import { sdk, getBackendUrl } from "@/lib/config"
...
export async function addToCartBulk({
  ...
  await fetch(
    `${getBackendUrl()}/store/carts/${cart.id}/line-items/bulk`,
    {
      method: "POST",
      headers,
...
export async function initiatePaymentSession(
  ...
  return sdk.store.payment
    .initiatePaymentSession(cart as HttpTypes.StoreCart, data, {}, headers)
    .then(async (resp) => {
      const cartCacheTag = await getCacheTag("carts")
      revalidateTag(cartCacheTag)
      return resp
...
export async function setAddresses(
  ...
  try {
    ...
    const data = {
      ...
      email: customer?.email || (formData.get("email") as string),
      billing_address: sameAsBilling
        ...
        : {
            ...
            address_2:
              (formData.get("billing_address.address_2") as string) || "",
            ...
            country_code: formData.get(
              "billing_address.country_code"
            ) as string,
            province:
              (formData.get("billing_address.province") as string) || "",
